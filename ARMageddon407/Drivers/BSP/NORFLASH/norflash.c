...
DMA_HandleTypeDef g_spi1_rx_dma_handler; /* SPI1 RX DMA句柄 */
DMA_HandleTypeDef g_spi1_tx_dma_handler; /* SPI1 TX DMA句柄 */

/* DMA传输完成标志 */
static volatile uint8_t g_spi_dma_rx_complete_flag = 0;
static volatile uint8_t g_spi_dma_tx_complete_flag = 0;
...
{
    /* 使能DMA时钟 */
    SPI1_RX_DMA_CLK_ENABLE();
    SPI1_TX_DMA_CLK_ENABLE();

    /* SPI1_RX DMA配置 */
    g_spi1_rx_dma_handler.Instance = SPI1_RX_DMA_STREAM;
    g_spi1_rx_dma_handler.Init.Channel = SPI1_RX_DMA_CHANNEL;
    g_spi1_rx_dma_handler.Init.Direction = DMA_PERIPH_TO_MEMORY;
    g_spi1_rx_dma_handler.Init.PeriphInc = DMA_PINC_DISABLE;
    g_spi1_rx_dma_handler.Init.MemInc = DMA_MINC_ENABLE;
    g_spi1_rx_dma_handler.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
    g_spi1_rx_dma_handler.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
    g_spi1_rx_dma_handler.Init.Mode = DMA_NORMAL;
    g_spi1_rx_dma_handler.Init.Priority = DMA_PRIORITY_MEDIUM;
    g_spi1_rx_dma_handler.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
    HAL_DMA_Init(&g_spi1_rx_dma_handler);
    
    /* 将DMA与SPI1关联 */
    __HAL_LINKDMA(&g_spi1_handler, hdmarx, g_spi1_rx_dma_handler);

    /* SPI1_TX DMA配置 */
    g_spi1_tx_dma_handler.Instance = SPI1_TX_DMA_STREAM;
    g_spi1_tx_dma_handler.Init.Channel = SPI1_TX_DMA_CHANNEL;
    g_spi1_tx_dma_handler.Init.Direction = DMA_MEMORY_TO_PERIPH;
    g_spi1_tx_dma_handler.Init.PeriphInc = DMA_PINC_DISABLE;
    g_spi1_tx_dma_handler.Init.MemInc = DMA_MINC_ENABLE;
    g_spi1_tx_dma_handler.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
    g_spi1_tx_dma_handler.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
    g_spi1_tx_dma_handler.Init.Mode = DMA_NORMAL;
    g_spi1_tx_dma_handler.Init.Priority = DMA_PRIORITY_MEDIUM;
    g_spi1_tx_dma_handler.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
    HAL_DMA_Init(&g_spi1_tx_dma_handler);
    
    /* 将DMA与SPI1关联 */
    __HAL_LINKDMA(&g_spi1_handler, hdmatx, g_spi1_tx_dma_handler);

    /* 配置DMA中断 */
    HAL_NVIC_SetPriority(SPI1_RX_DMA_IRQn, 0, 0);
    HAL_NVIC_EnableIRQ(SPI1_RX_DMA_IRQn);
    HAL_NVIC_SetPriority(SPI1_TX_DMA_IRQn, 0, 0);
    HAL_NVIC_EnableIRQ(SPI1_TX_DMA_IRQn);
}

/**
 * @brief       SPI初始化代码
 *   @note      主机模式,8位数据,禁止硬件片选,DMA方式
 * @param       无
 * @retval      无
 */
void spi1_init(void)
...
uint8_t spi1_read_write_byte(uint8_t txdata)
{
    ...
    /* 重置完成标志 */
    g_spi_dma_tx_complete_flag = 0;
    g_spi_dma_rx_complete_flag = 0;
    
    /* 使用DMA进行SPI收发 */
    HAL_SPI_TransmitReceive_DMA(&g_spi1_handler, &txdata, &rxdata, 1);
    
    /* 等待DMA传输完成 */
    while (!g_spi_dma_rx_complete_flag);
    
    return rxdata; /* 返回收到的数据 */
}

/**
 * @brief       SPI1读写多字节数据(DMA方式)
 * @param       txbuf   : 发送缓冲区
 * @param       rxbuf   : 接收缓冲区
 * @param       len     : 数据长度
 * @retval      无
 */
void spi1_read_write_buf_dma(uint8_t *txbuf, uint8_t *rxbuf, uint32_t len)
{
    /* 重置完成标志 */
    g_spi_dma_tx_complete_flag = 0;
    g_spi_dma_rx_complete_flag = 0;
    ...
    /* 使用DMA进行SPI收发 */
    HAL_SPI_TransmitReceive_DMA(&g_spi1_handler, txbuf, rxbuf, len);
...
/**
 * @brief       SPI1发送数据(DMA方式)
 * @param       txbuf   : 发送缓冲区
 * @param       len     : 数据长度
 * @retval      无
 */
void spi1_send_buf_dma(uint8_t *txbuf, uint32_t len)
{
    /* 重置完成标志 */
    g_spi_dma_tx_complete_flag = 0;
    
    /* 使用DMA进行SPI发送 */
    HAL_SPI_Transmit_DMA(&g_spi1_handler, txbuf, len);
    
    /* 等待DMA传输完成 */
    while (!g_spi_dma_tx_complete_flag);
}

/**
 * @brief       SPI1接收数据(DMA方式)
 * @param       rxbuf   : 接收缓冲区
 * @param       len     : 数据长度
 * @retval      无
 */
void spi1_recv_buf_dma(uint8_t *rxbuf, uint32_t len)
{
    /* 重置完成标志 */
    g_spi_dma_rx_complete_flag = 0;
    
    /* 使用DMA进行SPI接收 */
    HAL_SPI_Receive_DMA(&g_spi1_handler, rxbuf, len);
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
    if (hspi->Instance == SPI1_SPI)
    {
        g_spi_dma_tx_complete_flag = 1;
    }
}

/**
 * @brief       SPI1 RX DMA中断回调函数
 * @param       无
 * @retval      无
 */
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi)
{
    if (hspi->Instance == SPI1_SPI)
    {
        g_spi_dma_rx_complete_flag = 1;
    }
}

/**
 * @brief       SPI1 收发完成DMA中断回调函数
 * @param       无
 * @retval      无
 */
...
/**
 * @brief       获取SPI DMA传输完成状态
 * @param       无
 * @retval      1: 传输完成; 0: 传输未完成
 */
uint8_t spi1_dma_transfer_completed(void)
{
    return g_spi_dma_rx_complete_flag && g_spi_dma_tx_complete_flag;
}
