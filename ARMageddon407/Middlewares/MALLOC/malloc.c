...
/* --- 内部 SRAM (BANK 1) --- */
/* 内部 SRAM 比较特殊，通常由链接器自动分配，所以保留数组写法 */
static __ALIGNED(32) uint8_t mem1base[MEM1_MAX_SIZE]; 
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE]; 


/* --- 内部 CCM RAM --- */
#define CCM_BASE_ADDR   (0x10000000UL)

static uint8_t *const mem2base =
    (uint8_t *)CCM_BASE_ADDR;

static uint16_t *const mem2mapbase =
    (uint16_t *)((uintptr_t)CCM_BASE_ADDR + MEM2_MAX_SIZE);
...
struct _m_mallco_dev mallco_dev =
{
    ...
    /* 这里的赋值在 AC6 中是可以直接编译通过的 */
    { (uint8_t*)mem1base, (uint8_t*)mem2base, },      /* 内存池 */
    { (uint16_t*)mem1mapbase, (uint16_t*)mem2mapbase, }, /* 状态表 */
    { 0, 0, },            /* 内存管理未就绪 */
};
