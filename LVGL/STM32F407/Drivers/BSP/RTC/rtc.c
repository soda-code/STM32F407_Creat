 ...
 * @retval      无
 */
void rtc_get_time(RTC_STRUCT *RTC_Get)
{
	...
	RTC_Get->hour = rtc_time_handle.Hours;
	RTC_Get->min = rtc_time_handle.Minutes;
	RTC_Get->sec = rtc_time_handle.Seconds;
 ...
 * @retval      无
 */
void rtc_get_date(RTC_STRUCT *RTC_Get)
{
    RTC_DateTypeDef rtc_date_handle;

    HAL_RTC_GetDate(&g_rtc_handle, &rtc_date_handle, RTC_FORMAT_BIN);

	RTC_Get->year = rtc_date_handle.Year;
	RTC_Get->month = rtc_date_handle.Month;
	RTC_Get->date = rtc_date_handle.Date;
