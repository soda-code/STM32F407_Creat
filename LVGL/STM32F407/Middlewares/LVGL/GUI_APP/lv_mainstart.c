...
static void  timer_task( lv_event_t *e) 
{
	...
	lv_mem_monitor(&mon);
	usb_printf("Total Memory: %d bytes\r\n", mon.total_size);
	usb_printf("Free Memory: %d bytes\r\n", mon.free_size);
	usb_printf("Fragmentation: %d%%\r\n", mon.frag_pct);
 ...
 void LV_Main_Win(void)
{
    ...
    lv_obj_align(img, LV_ALIGN_TOP_MID, 0, 0);                                    /* 设置图片位置 */
	lv_obj_set_style_bg_opa(img,LV_OPA_10,0);
